// Package analysis propagates metric values up a Component tree using hierarchical
...
func CumSum(root *model.Component, metricKeys []string) {
	if root == nil {
		return
	}
...
func cumSumNode(c *model.Component, keys []string) map[string]float64 {
	if c.Metrics == nil {
...
func CumAvg(root *model.Component, totalKey, countKey, avgKey string) {
	if root == nil {
		return
	}
...
func cumAvgNode(c *model.Component, totalKey, countKey, avgKey string) {
	if c.Metrics == nil {
		c.Metrics = map[string]float64{}
	}
	for _, child := range c.Children {
