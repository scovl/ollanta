// Package domain defines the language-agnostic data model used across all Ollanta modules.
// Inspired by the LIM (Language Independent Model) from OpenStaticAnalyzer, every type
// here is the shared vocabulary between ollantarules, ollantascanner, and ollantaengine.
// A domain.Issue produced from a Go file is the same struct as one from a Python file.
package domain
...
// such as a step in a taint flow, a duplicated code fragment, or a data-flow path.
...
type Issue struct {
	...
	// issue tracker (ollantaengine) to match the same logical issue across scans
